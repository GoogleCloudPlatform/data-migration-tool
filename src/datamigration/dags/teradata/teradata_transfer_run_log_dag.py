...
BQ_TRANSFER_SUMMARY_TABLE_NAME = (
    f"{PROJECT_ID}.{BQ_LOGGING_DATASET_NAME}.dmt_teradata_transfer_run_summary"
)
BQ_TRANSFER_JOB_STATS_TABLE_NAME = (
    f"{PROJECT_ID}.{BQ_LOGGING_DATASET_NAME}.dmt_teradata_transfer_run_jobwise_details"
)
BQ_TRANSFER_TRACKING_TABLE_NAME = (
    f"{PROJECT_ID}.{BQ_LOGGING_DATASET_NAME}.dmt_teradata_transfer_tracking"
)
LOGGING_BUCKET_ID_PREFIX = "dmt-teradata-datamigration-logging"
...
def _kill_migration_agent(ti, **kwargs) -> None:
    """
    Kill Migration Agent
    """
    transfer_config_id = ti.xcom_pull(
        task_ids="load_parameters", key="transfer_config_id"
    )
    data = {
        "time": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
        "transfer_id": transfer_config_id,
        "params": {},
        "action": "kill",
    }
    payload = json.dumps(data).encode("utf-8")
    future = publisher.publish(topic_name, payload, ordering_key=transfer_config_id)
    future.result()
...
with models.DAG(
    ...
    kill_migration_agent = PythonOperator(
        task_id="kill_migration_agent",
        python_callable=_kill_migration_agent,
        dag=dag,
    )
    ...
    # dependencies
    (load_parameters >> kill_migration_agent)

    (
        kill_migration_agent
        >> get_dts_run_summary
        >> get_dts_run_logs
