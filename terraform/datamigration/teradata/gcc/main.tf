...
/* Copy Teradata data migration DAGs to Composer bucket */

resource "null_resource" "upload_td_datamigration_dags" {
  depends_on = [google_storage_bucket_iam_member.storage_object_admin]
  triggers = {
    datamigration_td_dag_hashes = jsonencode({
      for fn in fileset(var.datamigration_teradata_dag_source_path, "**") :
      fn => filesha256("${var.datamigration_teradata_dag_source_path}/${fn}")
    })
  }
